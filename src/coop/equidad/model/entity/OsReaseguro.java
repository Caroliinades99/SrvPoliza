...
/**
 * The persistent class for the S03221 database table.
 * 
 */
@Entity
@Table(name="S03221", schema="osiris")
@NamedQueries({
	@NamedQuery(name="OsReaseguro.findAll", query="SELECT o FROM OsReaseguro o"),
	@NamedQuery(name="OsReaseguro.findByPoliza", query="SELECT r FROM OsReaseguro r "
			+ "WHERE r.id.sucur = :sucur AND r.id.codpla = :codpla AND r.id.certif = :certif AND r.id.orden = :orden")
})
public class OsReaseguro implements Serializable {
