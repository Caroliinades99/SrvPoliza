...
/**
 * The persistent class for the S03033 database table.
 * 
 */
@Entity
@Table(name="S03033", schema="osiris")
@NamedQueries({
	@NamedQuery(name="OsCoberturaPz.findAll", query="SELECT o FROM OsCoberturaPz o"),
	@NamedQuery(name="OsCoberturaPz.findByPoliza", query="SELECT c FROM OsCoberturaPz c "
			+ "WHERE c.id.sucur = :sucur AND c.id.codpla = :codpla AND c.id.certif = :certif AND c.orden = :orden")
})
public class OsCoberturaPz implements Serializable {
